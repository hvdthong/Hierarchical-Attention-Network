...
class WordRNN(nn.Module):
    def __init__(self, vocab_size, embedsize, batch_size, hid_size):
        ...
        self.w_context_vector = nn.Parameter(torch.randn(self.embedsize).float())
        self.softmax = nn.Softmax(dim=1)


    def forward(self, inp, hid_state):
        emb_out = self.embed(inp)
        batch, sent_leng = emb_out.shape[0], emb_out.shape[1]
        ...
        Hw = F.tanh(self.wordattn(out_state))
        w_score = self.softmax(Hw.matmul(self.w_context_vector))
        out_state = out_state.mul(w_score)
        x = torch.sum(out_state, dim=1)
        return x, hid_state
